...
function tonumber_plusfloat(num)
   local strval = tostring(num)
   local numval = tonumber(num)
   if strval:match("%.") then
     numval = numval * 1.0
   end
...
function OBJDEF:asNumber(item)
   local holder = {
      ...
      N = tonumber_plusfloat(item), -- N is the number as a Lua number.
   }
...
local function grok_number(self, text, start, options)
   ...
   -- fix for numbers like 10.0 converted into int
   if decimal_part:len() > 0 then
      as_number = as_number * 1.0
   end
